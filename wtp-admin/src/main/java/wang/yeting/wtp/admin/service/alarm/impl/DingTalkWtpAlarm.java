 ...
 */
@Component
@ConditionalOnProperty(name = "dingtalk.alarm.enabled", havingValue = "true")
public class DingTalkWtpAlarm implements WtpAlarm {
    ...
    private Boolean sendMsg(String text) {
        try {
            ...
            DingTalkMsg.At at = dingTalkMsg.new At();
            // @某人  手机号
            //at.setAtMobiles()
            // @全部
            at.setIsAtAll(true);
            DingTalkMsg.Text textContent = dingTalkMsg.new Text();
            textContent.setContent(text);
            dingTalkMsg.setText(textContent);
            dingTalkMsg.setAt(at);
            //发送
            String post = HttpUtil.post(webhook, JSONUtil.toJsonStr(dingTalkMsg));

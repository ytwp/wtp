  ...
  <div class="app-container">
    <el-row :gutter="3">
      ...
      <el-col :span="5">
        ...
        <el-select v-model="pageQuery.clusterId" :disabled="clusterIdOptions.length <= 0" filterable placeholder="可搜索"
      ...
      <el-col :span="5">
        <el-button :disabled="!pageQuery.clusterId" v-if="check(pageQuery.appId,'INSERT')" type="primary" @click="newWtpVisible = true">New
          Wtp</el-button>
      </el-col>
      <el-col :span="5">
        <el-button :disabled="!pageQuery.clusterId" type="primary" @click="registryVisibleBtn">{{ 'OnLine 机器( ' + wtpRegistryList.length + ' )' }}</el-button>
      </el-col>
    </el-row>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;margin-top: 20px;">
      ...
      <el-table-column align="center" label="Actions">
        <template slot-scope="scope">
          <el-button-group>
            <el-button v-if="check(scope.row.appId,'UPDATE')" type="primary" @click="updateWtpBtn(scope.row.wtpId)">编辑</el-button>
          ...
          <el-popconfirm @onConfirm="delWtpBtn(scope.row.wtpId)" confirmButtonText='好的' cancelButtonText='不用了' icon="el-icon-info"
            iconColor="red" title="删除后不可恢复,确定删除吗？">
            <el-button slot="reference" v-if="check(scope.row.appId,'DELETE')" type="danger">删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="修改 Wtp" :visible.sync="updateWtpVisible">
      <el-form :model="wtp">
        ...
        <el-alert v-if="wtp.queueName && wtp.queueName.indexOf('ResizableCapacity') < 0" :closable="false" title="（谨慎）当前 queueName 不支持动态修改队列长度 ，需重启项目，才能生效"
          type="warning" close-text="知道了" />
        <el-form-item label="queueCapacity" label-width="140px">
          <el-input v-model="wtp.queueCapacity" type="number" autocomplete="off" />
        </el-form-item>
        <el-alert v-if="wtpRegistryList.length > 0" :closable="false" title="（谨慎）修改 queueName 需重启项目，才能生效" type="warning"
          close-text="知道了" />
        <el-form-item label="queueName" label-width="140px">
    ...
    methods: {
      ...
      report(appId, clusterId, name) {
        ...
        findWtpRegistry(this.pageQuery).then((response) => {
          let wtpRegistryList = response.data
